...
#for row in rows:
#    print (row)
eFM_qry = """SELECT scplaylst.chan_cd,   
                 scplaylst.trff_ymd,   
                 scplaylst.trff_seq,   
                 scplaylst.trff_time,   
                 scplaylst.trff_run,   
           ...
           WHERE ( scplaylst.chan_cd = 'CH_E' ) AND  
...
tableResult = pd.read_sql(eFM_qry, cnxn)
#copia(tableResult)
...
#df.to_json(PATH+'db_efm_{}.json'.format(today),orient='table')
#df.to_json(PATH+'db_efm.json',orient='table')
...
df = df[~df['pgm_id'].str.contains("PG2060117E")]
df = df[~df['pgm_id'].str.contains("PG2060098E")]
df = df[~df['pgm_id'].str.contains("PG2061304E")]
df = df[~df['pgm_id'].str.contains("PG2061303E")]
#df
